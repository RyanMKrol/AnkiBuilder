...
// ttsText, so `speechText` handed the digits straight to the voice.
test("flags a card whose target has a digit and no ttsText", () => {
  ...
  assert.match(found[0].cause, /no ttsText/);
  assert.equal(found[0].field, "spoken");
});

test("a spelled-out ttsText clears it", () => {
  const found = findUnreadableNumbers(
    [card("a", "2025ねんに", { ttsText: "にせんにじゅうごねんに" })],
    "ja",
  );
  assert.deepEqual(found, []);
});

// A different mistake with a different fix: someone wrote a ttsText but left a digit in it.
test("a ttsText that still contains a digit is flagged, and says so", () => {
  const [found] = findUnreadableNumbers([card("a", "2025ねんに", { ttsText: "2025ねんに" })], "ja");
  assert.equal(found.cause, "ttsText still contains a digit");
});
...
test("the report names each card, its text and the cause", () => {
  ...
  assert.match(report, /a: 5じに — no ttsText/);
});

// The romaji is what the learner reads to know how to SAY the card, so a digit in it is wrong on its
// own terms. This arm also catches the stale case: a ttsText filled in later without regenerating the
...
test("a digit in the romaji is flagged even when the spoken text is clean", () => {
  const [found] = findUnreadableNumbers(
    [card("a", "2025ねんに", { ttsText: "にせんにじゅうごねんに", pronunciation: "2025-nen ni" })],
    "ja",
  );
...
test("a card with a numeric target, a spelled-out ttsText and clean romaji passes", () => {
  assert.deepEqual(
