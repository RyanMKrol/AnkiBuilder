...
**Write the `english` side in natural sentence case.** Capitalize the first word (and proper nouns) as you would writing normal English — even for a bare vocabulary word or fragment. `"Department store"`, `"Coffee"`, `"How much?"`, `"By means of (particle)"`, `"That's right"` — never lowercased clips like `"department store"`, `"how much"`, or `"by means of (particle)"`. Punctuate full sentences and questions normally (`.` / `?`). This is only about the English gloss reading like real English; leave the `target` verbatim.
