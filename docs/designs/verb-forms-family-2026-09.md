...
Two gaps reported by the deck's owner, investigated together, fixed as two addition batches against
the Japanese for Busy People Book 1 collection. The card data is in
...
## How both batches ship

Both went in as additions: every card carries `addition: "verb-dictionary-forms"` or
`addition: "family-vocabulary"` and neither review flag, so `shippableCards()` holds it out of the
`.apkg` and the AnkiConnect deliver until it passes the per-card gates at `/additions/epub/...`. The
three destination units keep their own `reviewed` and `done` sign-off untouched. The package was
rebuilt after the merge and contains 2366 notes, the same count as before, which is the check that
the hold is working.

No audio exists for any of the forty-five cards yet. That is deliberate: the content gate comes first
so TTS credits are not spent on a card that might be cut.

## What was written back

- `card-authoring-rules.md` gained "Every verb gets its citation form, whatever form the source
  printed", stated language-agnostically, with the routing rule, the note-content rule, the
  paradigm-as-an-image warning and an audit recipe.
