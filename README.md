...
| **audio**     | Each term gets a spoken recording, via ElevenLabs. A card may carry an optional `reading` (a phonetic spelling in the target script) that TTS speaks instead of `target`. Languages with an "alt audio" config (Japanese appends `。`) get two recordings — the with-`。` take is the default and the plain take is the alt, offered in the audio review to switch to or drop. |
